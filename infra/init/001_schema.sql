...
CREATE INDEX idx_executions_analysis
    ON executions(experiment_id, model_id, pollution_level);
...
-- View: Métricas agregadas por modelo e nível de poluição
...
GROUP BY experiment_id, model_name, provider_name, parameter_count, pollution_level
ORDER BY model_name, pollution_level;

-- View: Comparação entre modelos
...
SELECT
    ...
    ROUND(AVG(latency_ms)::numeric, 2) AS avg_latency_ms
FROM v_experiment_results
GROUP BY experiment_id, model_name, parameter_count;
...
-- Inserir tools do cenário de ações
...
-- Inserir template de prompt padrão
INSERT INTO prompt_templates (name, description, system_prompt, user_prompt_template, context_template, expected_tool_id, variables_schema) VALUES
    ('stock_price_query',
     'Pergunta sobre preço atual de ação com contexto de relatório de investimentos',
